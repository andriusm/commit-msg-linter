...
let instructions = """
    ...
    - It only records a dependency, language, or runtime version change (patch, minor, or major) \
    without saying why that bump was required—e.g. "Update Go patch version", "bump go", \
    "upgrade lodash" are insufficient on their own; the message must state the trigger (security \
    advisory, upstream bugfix you need, broken build, compliance policy, incident, etc.)
    ...
    nothing about the problem, goal, or constraint behind adjusting it. "Update Go patch version" \
    and "bump go" fail for the same reason: they describe the version motion, not why this bump \
    is happening now.
